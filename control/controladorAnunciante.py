...
class ControladorAnunciante():
    ...
    def cadastrar_anunciante(self, nome, cpf, numero_cartao_credito, cvv_cartao_credito, senha):
        anunciante = Anunciante(
            nome, cpf, numero_cartao_credito, cvv_cartao_credito, senha)
    ...
    def abrir_tela_anunciante(self):
        ...
        kwargs = {}
        while True:
            result = self.__tela_cadastro_anunciante.comecar(
                erro=erro, **kwargs)
            if result["result"]:
    ...
    def validar_dados_cadastro(self, nome, cpf, numero_cartao_credito, cvv_cartao_credito, senha, confirmacao):
        ...
        verificacoes = [
            ...
            (lambda: len(numero_cartao_credito) >= 13 and len(numero_cartao_credito)
             <= 16, "O número do cartão de crédito deve ter de 13 à 16 dígitos"),
            (lambda: numero_cartao_credito.isdecimal(),
             "O número do cartão de crédito deve conter apenas números"),

            (lambda: len(cvv_cartao_credito) >= 3 and len(cvv_cartao_credito)
             <= 4, "O CVV do cartão de crédito deve ter de 3 à 4 dígitos"),
            (lambda: cvv_cartao_credito.isdecimal(),
             "O CVV do cartão de crédito deve conter apenas números")
        ]
    ...
    def abrir_tela_login(self):
        ...
        while True:
            ...
            anunciante = self.__dao_anunciante.pegar_anunciante(
                **dados["result"])
            if not anunciante:
    ...
    def abrir_tela_anunciar_curso(self, anunciante):
        acao_tela_curso = self.__controlador_curso.abrir_tela_anunciar_curso(
            anunciante.cursos)

        if (not acao_tela_curso["result"]["curso"]):
            return acao_tela_curso
        if (acao_tela_curso["result"]):
